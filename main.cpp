...
public:
    ...
    { // make a version ready to be commited
        std::ifstream fin(file);
        if (!fin)
        {
            std::cerr << "File not found\n";
            return;
        }                               // if file is empty return file not found
    ...
    {                                    // msg is the message the wrriten by the user
        std::ifstream reader(indexPath); // open file for reading
        std::vector<std::string> lines;
        std::string line;
        while (std::getline(reader, line))
        { // read from the file and push it in to the vector
        ...
        for (auto &l : lines)
        { // it writes each line in the lines
            commitContent << l << "\n";
        }
    ...
    void merge(const std::string &branchName)
    {
        ...
        std::ifstream headIn(headPath);
        std::getline(headIn, currentCommitHash);
        headIn.close();
        std::ifstream branchIn(branchPath);
        std::getline(branchIn, targetCommitHash);
        branchIn.close();
    ...
    void diff(const std::string &commit1, const std::string &commit2)
    {
        ...
        for (const auto &[f, _] : files1)
            allFiles.insert(f);
        for (const auto &[f, _] : files2)
            allFiles.insert(f);
...
int main()
{
    ...
    while (true)
    {
        ...
        else if (cmd == "checkout")
        {
            ss >> arg;
            if (arg.empty())
                cout << "Usage: checkout <branch-name>\n";
            else
                mg.checkoutBranch(arg);
        }
